...
import { aiInsight, sensorMetrics } from "../../data/mockDormData";
import { TrendLine } from "../ui/TrendLine";

export function RecommendationCard() {
  const co2Metric = sensorMetrics.find((metric) => metric.key === "co2");
  ...
  return (
    <div className="rounded-[22px] border border-amber-100/80 bg-[linear-gradient(135deg,rgba(255,251,235,0.82),rgba(255,255,255,0.62))] p-4">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-600">{aiInsight.recommendation}</p>
      <div className="mt-4 h-12 rounded-2xl border border-white/70 bg-white/58 p-2">
        <TrendLine values={co2Metric?.history ?? [1]} color="#F59E0B" height={36} />
      </div>
    </div>
  );
}
