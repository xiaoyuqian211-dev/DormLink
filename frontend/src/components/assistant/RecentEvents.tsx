...
import type { AssistantRecentEvent } from "../../utils/assistantTelemetry";

export function RecentEvents({ events }: { events: AssistantRecentEvent[] }) {
  return (
    <div className="rounded-[22px] border border-slate-300/18 bg-white/58 p-4">
      ...
      <div className="mt-4 space-y-3">
        {events.length ? (
          ...
          <p className="text-sm leading-6 text-slate-500">暂无真实历史事件。</p>
        )}
      </div>
    </div>
  );
